...
export async function publisher() {
  const readme = cleanReadme(md);
  const tsUtil = 'Chainable Options';
  try {
    genArticle(readme, tsUtil);
  } catch (error) {
    console.log(`🚀 > file: index.ts:47 > publisher > error:`, error);
  }
}

// 清理md
function cleanReadme(md: string) {
  const r = /\n?<!--info-\w*-start-->.*<!--info-\w*-end-->\n?/gi;
